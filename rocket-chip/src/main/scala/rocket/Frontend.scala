...
class FrontendIO(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  val progress = Bool(OUTPUT)
   ...
   //forpac
  //reversed by bundle Flipped tag
  val ras = Input(UInt(64.W))
}
  ...
  if (usingBTB) {
    ...
    when (btb.io.resp.valid && btb.io.resp.bits.taken) {
      ...
      predicted_taken := Bool(true)
       ...
       //for pac
    //(BTB.io.ras_head.bits: IO[UInt<39>])
    //io.cpu.ras := btb.io.ras_head.bits.pad(64)
    io.cpu.ras := Cat(0.U(25.W),btb.io.ras_head.bits)
